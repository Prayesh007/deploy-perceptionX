...
async def process_file(file: UploadFile = File(...), background_tasks: BackgroundTasks = None):
    ...
    insert_result = await files_collection.insert_one(file_doc)
...
async def process_file_background(file_id, file_type):
    for attempt in range(5):
        ...
        if doc:
            break
        import asyncio
        print(f"\u23F3 Waiting for file to be retrievable... Attempt {attempt+1}")
        await asyncio.sleep(1)
    else:
        print("\u274C File not found in DB after retries")
        return
...
async def process_video(file_id, data):
    ...
    temp_out = input_path.replace(".mp4", "_out.avi")
    final_mp4 = temp_out.replace(".avi", ".mp4")
    ...
    subprocess.run(["ffmpeg", "-y", "-i", temp_out, "-vcodec", "libx264", "-crf", "23", "-preset", "fast", final_mp4])
